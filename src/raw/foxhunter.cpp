...
void ascendingBeeps() {
    ...
    if (buzzerEnabled) {
        boardLedcTone(1000);  // Set to 1kHz for consistency with proximity beeps
        boardLedcSetDuty(0);  // Make sure buzzer is completely off
    }
...
void threeSameToneBeeps() {
    ...
    for (int i = 0; i < 3; i++) {
        if (buzzerEnabled) {
            boardLedcTone(1000); // Same 1kHz tone as proximity beeps
            boardLedcSetDuty(BUZZER_DUTY);
        }
        ledOn();
        delay(100);
